...
export const ThemeProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      root.style.setProperty('--font-size-base', accessibilityOptions.fontSize[accessibility.fontSize].size);
      root.style.setProperty('--contrast-filter', accessibilityOptions.contrast[accessibility.contrast].filter);
      ...
      localStorage.setItem('pdf-merger-accessibility', JSON.stringify(accessibility));
    }
...
const ThemeAccessibilitySettings = ({ isOpen, onClose }) => {
  const { currentTheme, themes, accessibility, updateTheme, updateAccessibility, resetSettings } = useTheme();
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-[var(--color-border,#A0AEC0)]/20">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-[var(--color-text,#E1E6EB)]">Appearance & Accessibility</h2>
            </div>
              ...
              >
                ...
                {activeTab === 'themes' && (
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-lg font-semibold text-[var(--color-text,#E1E6EB)] mb-4">Choose Theme</h3>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3 mb-2">
                                ...
                                <h4 className="font-semibold text-[var(--color-text,#E1E6EB)]">{theme.name}</h4>
                              </div>
                              <p className="text-[var(--color-textSecondary,#A0AEC0)] text-sm">{theme.description}</p>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {Object.entries(accessibilityOptions.fontSize).map(([key, option]) => (
                          <button
                            key={key}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        {Object.entries(accessibilityOptions.contrast).map(([key, option]) => (
                          <button
                            key={key}
                    ...
                    <div className="space-y-4">
                      ...
                      {[
                        { key: 'reduceMotion', label: 'Reduce Motion', description: 'Minimize animations and transitions', icon: MousePointer },
                        { key: 'soundEnabled', label: 'Sound Effects', description: 'Enable audio feedback', icon: Volume2 },
                        { key: 'highContrastMode', label: 'High Contrast Mode', description: 'Enhanced visual contrast', icon: Contrast },
                        { key: 'focusIndicators', label: 'Focus Indicators', description: 'Enhanced keyboard navigation', icon: Keyboard },
                        { key: 'keyboardNavigation', label: 'Keyboard Navigation', description: 'Full keyboard support', icon: Keyboard }
                      ].map(option => {
                        const Icon = option.icon;
                        return (
                          <div key={option.key} className="flex items-center justify-between p-4 bg-[var(--color-secondary,#151B24)] rounded-lg border border-[var(--color-border,#A0AEC0)]/20">
                            <div className="flex items-center gap-3">
                              <Icon className="w-5 h-5 text-[var(--color-primary,#00A99D)]" />
                              <div>
                                <h4 className="font-medium text-[var(--color-text,#E1E6EB)]">{option.label}</h4>
                                <p className="text-[var(--color-textSecondary,#A0AEC0)] text-sm">{option.description}</p>
                              </div>
                            </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="bg-[var(--color-secondary,#151B24)] rounded-xl border border-[var(--color-border,#A0AEC0)]/20 p-6">
                        <h4 className="font-semibold text-[var(--color-text,#E1E6EB)] mb-4">Preview</h4>
                        <div className="bg-[var(--color-background,#1B212C)] rounded-lg border border-[var(--color-border,#A0AEC0)]/20 p-4">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                              <h5 className="font-medium text-[var(--color-text,#E1E6EB)]">Sample Component</h5>
                              <p className="text-[var(--color-textSecondary,#A0AEC0)] text-sm">This shows how your settings look</p>
                            </div>
                          </div>
                    ...
                    {/* Color Blind Support */}
                    <div>
                      <h3 className="text-lg font-semibold text-[var(--color-text,#E1E6EB)] mb-4">Color Blind Support</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-[var(--color-secondary,#151B24)] rounded-lg border border-[var(--color-border,#A0AEC0)]/20 p-4">
                          <h4 className="font-medium text-[var(--color-text,#E1E6EB)] mb-2">Protanopia Simulation</h4>
                          <p className="text-[var(--color-textSecondary,#A0AEC0)] text-sm">Red-green color blindness support</p>
                        </div>
                        <div className="bg-[var(--color-secondary,#151B24)] rounded-lg border border-[var(--color-border,#A0AEC0)]/20 p-4">
                          <h4 className="font-medium text-[var(--color-text,#E1E6EB)] mb-2">Deuteranopia Simulation</h4>
                          <p className="text-[var(--color-textSecondary,#A0AEC0)] text-sm">Red-green color blindness support</p>
                        </div>
                      </div>
                    </div>
...
export default ThemeAccessibilitySettings;
