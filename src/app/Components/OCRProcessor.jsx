...
const OCRProcessor = ({ onNavigate }) => {
  ...
  const { showToast, ToastContainer } = useToast();useEffect(() => {
  ...
  if (!mounted || !pdfjs) {
    return (
      <div className="min-h-screen bg-[#1B212C] flex items-center justify-center p-4">
        <div className="text-center max-w-sm">
          <div className="w-12 h-12 sm:w-16 sm:h-16 border-4 border-[#00A99D] border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <div className="text-[#E1E6EB] text-lg sm:text-xl font-medium mb-2">Loading OCR processor...</div>
          <div className="text-[#A0AEC0] text-sm">Please wait while we initialize the text extraction engine</div>
        </div>
      </div>
    );
  }
  ...
  const processOCR = async () => {
    ...
    try {
      ...
      for (let fileIndex = 0; fileIndex < files.length; fileIndex++) {
        ...
        if (file.type === 'application/pdf') {          // Process PDF file with better error handling
          try {
        ...
        if (processingOptions.extractStructuredData && fileResult.searchableText) {
          try {
            ...
            console.error('Failed to extract structured data:', structuredError);
          }
        }

        allResults.push(fileResult);
      }      setResults(allResults);
      showToast(`Successfully processed ${allResults.length} file(s)!`, 'success');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#1B212C] text-white">
      ...
      <div className="bg-[#1B212C] border-b border-[#A0AEC0]/20 sticky top-0 z-40">        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 lg:gap-8">          {/* Upload and Processing Panel */}
          <div className="xl:col-span-2 space-y-4 lg:space-y-6">
            ...
            >
              ...
              <div className="border-2 border-dashed border-[#00A99D]/30 rounded-lg p-8 text-center">
                ...
                </div>              </div>              {/* File Type Summary */}
              {files.length > 0 && (
                <div className="mt-6 p-3 sm:p-4 bg-[#2A3441] rounded-lg">
                  <h3 className="font-medium mb-3 text-sm sm:text-base">File Analysis Summary</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 text-sm">
                    <div className="flex items-center justify-between sm:justify-start sm:space-x-2">
                      ...
                      <span className="font-medium">{fileTypes.filter(ft => ft.type === 'pdf').length}</span>
                    </div>
                    <div className="flex items-center justify-between sm:justify-start sm:space-x-2">
                      <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium">{fileTypes.filter(ft => ft.type === 'image').length}</span>
                    </div>
                    <div className="flex items-center justify-between sm:justify-start sm:space-x-2">
                      <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium">{fileTypes.filter(ft => ft.type === 'unsupported').length}</span>
                    </div>
                  </div>
              ...
              {files.length > 0 && (
                <div className="mt-6 space-y-2">
                  ...
                  {files.map((file, index) => {
                    const fileTypeInfo = fileTypes[index] || detectFileType(file);
                    const isUnsupported = fileTypeInfo.type === 'unsupported';
                    
                    return (
                      <div key={index} className="bg-[#1B212C] p-3 rounded-lg space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            <div className={`w-8 h-8 rounded flex items-center justify-center ${
                              isUnsupported ? 'bg-red-500/20' : 'bg-[#00A99D]/20'
                            }`}>
                              <svg className={`w-4 h-4 ${
                                isUnsupported ? 'text-red-400' : 'text-[#00A99D]'
                        ...
                        <div className={`p-2 rounded text-sm ${
                          ...
                          <p className="font-medium mb-1">
                            {isUnsupported ? '⚠️ Unsupported File' : `✓ ${fileTypeInfo.type.toUpperCase()} detected`}
                          </p>
            ...
            >
              <h2 className="text-lg font-semibold mb-4">Processing Options</h2>              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                <label className="flex items-start space-x-3 p-2 hover:bg-[#1B212C]/50 rounded">
                  <input
                    type="checkbox"
                    ...
                    className="rounded border-[#A0AEC0]/20 bg-[#1B212C] text-[#00A99D] focus:ring-[#00A99D] focus:ring-offset-0 mt-0.5"
                  />
              ...
              </div>{processingOptions.preprocessImage && (
                <div className="mt-4 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-[#1B212C] p-3 rounded-lg">
                      <label className="block text-sm font-medium mb-3">
                        Contrast: <span className="text-[#00A99D]">{processingOptions.contrast}%</span>
                      </label>
                      <input
                        type="range"
                        min="50"
                    ...
                    <div className="bg-[#1B212C] p-3 rounded-lg">
                      <label className="block text-sm font-medium mb-3">
                        Brightness: <span className="text-[#00A99D]">{processingOptions.brightness}%</span>
                      </label>
                      <input
                        type="range"
                        min="50"
                ...
                <button
                  ...
                  className="bg-[#00A99D] hover:bg-[#00A99D]/90 disabled:bg-[#A0AEC0]/20 disabled:cursor-not-allowed text-white px-6 py-3 sm:py-2 rounded-lg transition-colors flex items-center justify-center space-x-2 text-sm sm:text-base"
                >
                  {isProcessing ? (
                    <>
          ...
          <div className="space-y-4 lg:space-y-6">
              ...
              >                <h2 className="text-lg font-semibold mb-4">Search Text</h2>
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                  <input
                    ...
                    className="flex-1 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg px-3 py-3 sm:py-2 text-white placeholder-[#A0AEC0] focus:outline-none focus:ring-2 focus:ring-[#00A99D] focus:border-transparent text-sm sm:text-base"
                  />
                  <button
                    onClick={handleSearch}
                    className="bg-[#00A99D] hover:bg-[#00A99D]/90 text-white px-6 py-3 sm:py-2 rounded-lg transition-colors whitespace-nowrap text-sm sm:text-base"
                  >
                    Search
                  </button>
                </div>                {/* Search Results */}
                    ...
                    {searchResults.map((result, index) => (
                      <div key={index} className="bg-[#1B212C] p-3 rounded-lg">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2 space-y-1 sm:space-y-0">
                          <span className="font-medium text-sm break-words">{result.fileName}</span>
                          <span className="text-xs text-[#A0AEC0] whitespace-nowrap">Page {result.pageNumber}</span>
                        </div>
                        <p className="text-sm text-[#A0AEC0] break-words">{result.context}</p>
            ...
            {results.length > 0 && (
              <motion.div
                ...
                transition={{ delay: 0.1 }}
              >
                <h2 className="text-lg font-semibold mb-4">Processing Results</h2>                <div className="space-y-3 sm:space-y-4">
                  {results.map((result, index) => (
                    <div key={index} className="bg-[#1B212C] p-3 sm:p-4 rounded-lg">
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 space-y-2 sm:space-y-0">
                        <h3 className="font-medium text-sm sm:text-base break-words pr-0 sm:pr-4">{result.fileName}</h3>
                        <span className="text-sm text-[#A0AEC0] whitespace-nowrap">
                      ...
                      {result.extractedData && (
                        <div className="text-sm mb-3">
                          <p className="text-[#00A99D] font-medium mb-2">Extracted Data:</p>
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <div className="flex justify-between">
                              <span>Emails:</span>
                              <span className="font-medium">{result.extractedData.emails?.length || 0}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Phone:</span>
                              <span className="font-medium">{result.extractedData.phones?.length || 0}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Dates:</span>
                              <span className="font-medium">{result.extractedData.dates?.length || 0}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>URLs:</span>
                              <span className="font-medium">{result.extractedData.urls?.length || 0}</span>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="mt-3">
                        <button 
                          ...
                          className="w-full sm:w-auto text-sm text-[#00A99D] hover:text-[#00A99D]/80 transition-colors py-2 sm:py-0 text-center sm:text-left"
                        >
                          {expandedResults.includes(index) ? '▼ Hide Details' : '▶ View Details'}
                        </button>
                      ...
                      <AnimatePresence>
                        {expandedResults.includes(index) && (
                          <motion.div
                            ...
                            className="mt-4 border-t border-[#A0AEC0]/20 pt-4 overflow-hidden"
                          >
                            {/* Page-by-page results */}
                            <div className="mb-4 sm:mb-6">
                              <h4 className="text-sm sm:text-base font-medium text-[#00A99D] mb-3">Page Details</h4>
                              <div className="space-y-3 max-h-64 sm:max-h-80 overflow-y-auto">
                                {result.pages.map((page, pageIndex) => (
                                  <div key={pageIndex} className="bg-[#2A3441] p-3 sm:p-4 rounded-lg">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 space-y-2 sm:space-y-0">
                                      <span className="font-medium text-sm sm:text-base">Page {page.pageNumber}</span>
                                      <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-3">
                                        <span className={`px-2 py-1 rounded text-xs text-center ${
                                          page.success ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'
                                        }`}>
                                          {page.success ? '✓ Success' : '✗ Failed'}
                                        </span>
                                        {page.confidence && (
                                          <span className="text-[#A0AEC0] text-xs text-center sm:text-left">
                                            {Math.round(page.confidence)}% confidence
                                          </span>
                                        )}
                                      </div>
                                    </div>
                                    <div className="text-[#E1E6EB] bg-[#1B212C] p-3 rounded text-xs sm:text-sm max-h-24 sm:max-h-32 overflow-y-auto">
                            ...
                            {/* Full extracted text */}
                            <div className="mb-4 sm:mb-6">
                              <h4 className="text-sm sm:text-base font-medium text-[#00A99D] mb-3">Full Extracted Text</h4>
                              <div className="bg-[#2A3441] p-3 sm:p-4 rounded-lg">
                                <div className="max-h-48 sm:max-h-64 overflow-y-auto">
                                  <pre className="whitespace-pre-wrap text-[#E1E6EB] text-xs sm:text-sm break-words">
                                    {result.searchableText || 'No text extracted'}
                                  </pre>
                                </div>
                              </div>
                            </div>
                            ...
                            {result.extractedData && (
                              <div className="mb-4 sm:mb-6">
                                <h4 className="text-sm sm:text-base font-medium text-[#00A99D] mb-3">Structured Data</h4>
                                <div className="bg-[#2A3441] p-3 sm:p-4 rounded-lg space-y-3">
                                  {result.extractedData.emails?.length > 0 && (
                                    <div>
                                      <span className="font-medium text-[#E1E6EB] text-sm block mb-1">Emails:</span>
                                      <div className="text-[#A0AEC0] text-xs sm:text-sm break-all">
                                  ...
                                  {result.extractedData.phones?.length > 0 && (
                                    <div>
                                      <span className="font-medium text-[#E1E6EB] text-sm block mb-1">Phone Numbers:</span>
                                      <div className="text-[#A0AEC0] text-xs sm:text-sm break-all">
                                  ...
                                  {result.extractedData.dates?.length > 0 && (
                                    <div>
                                      <span className="font-medium text-[#E1E6EB] text-sm block mb-1">Dates:</span>
                                      <div className="text-[#A0AEC0] text-xs sm:text-sm break-all">
                                  ...
                                  {result.extractedData.urls?.length > 0 && (
                                    <div>
                                      <span className="font-medium text-[#E1E6EB] text-sm block mb-1">URLs:</span>
                                      <div className="text-[#A0AEC0] text-xs sm:text-sm break-all">
                            ...
                            <div className="mt-4 pt-4 border-t border-[#A0AEC0]/20">
                              <h4 className="text-sm font-medium text-[#E1E6EB] mb-3">Actions</h4>
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">                              <button
                                onClick={() => {
                                  navigator.clipboard.writeText(result.searchableText);
                                  showToast('Text copied to clipboard!', 'success');
                                }}
                                className="bg-[#00A99D]/20 hover:bg-[#00A99D]/30 text-[#00A99D] px-4 py-3 sm:py-2 rounded-lg text-sm transition-colors flex items-center justify-center space-x-2"
                              >
                                  <span>📋</span>
                                  <span>Copy Text</span>
                                </button>
                                <button                                onClick={() => {
                                  ...
                                  className="bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 px-4 py-3 sm:py-2 rounded-lg text-sm transition-colors flex items-center justify-center space-x-2"
                                >
                                  <span>💾</span>
                                  <span>Save Text</span>
                                </button>
                                {result.extractedData && (
                                  <button                                    onClick={() => {
                                      const content = JSON.stringify(result.extractedData, null, 2);
                                      ...
                                      a.click();
                                      URL.revokeObjectURL(url);
                                      showToast('Data file saved successfully!', 'success');
                                    }}
                                    className="bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 px-4 py-3 sm:py-2 rounded-lg text-sm transition-colors flex items-center justify-center space-x-2"
                                  >
                                    <span>📊</span>
                                    <span>Save Data</span>
                                  </button>
