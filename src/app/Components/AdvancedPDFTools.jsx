            ...
            switch (splitSettings.mode) {
              case SPLIT_MODES.ALL_PAGES:
                ...
                if (!splitSettings.pageRanges.trim()) {
                  result = { success: false, error: 'Please specify page ranges (e.g., 1-3,5-7)' };
                } else {
                  // Get PDF info to get total page count for validation
                  const pdfInfo = await getPDFInfo(file);
                  if (!pdfInfo.success) {
                    result = { success: false, error: 'Failed to read PDF information' };
                  } else {
                    const ranges = parsePageRanges(splitSettings.pageRanges, pdfInfo.info.pageCount);
                    if (ranges.length === 0) {
                      result = { success: false, error: 'No valid page ranges found. Please check your input.' };
                    } else {
              ...
              case SPLIT_MODES.SPECIFIC_PAGES:
                if (!splitSettings.specificPages.trim()) {
                  result = { success: false, error: 'Please specify page numbers (e.g., 1,3,5)' };
                } else {
                  // Get PDF info to get total page count for validation
                  const pdfInfo = await getPDFInfo(file);
                  if (!pdfInfo.success) {
                    ...
                    const ranges = parsePageRanges(splitSettings.specificPages, pdfInfo.info.pageCount);
                    if (ranges.length === 0) {
                      result = { success: false, error: 'No valid page numbers found. Please check your input.' };
                    } else {
                      result = await splitByPageRanges(file, ranges, {
                        preserveBookmarks: splitSettings.preserveBookmarks
                      });
              ...
              case SPLIT_MODES.CHUNKS:
                result = await splitByPageChunks(file, splitSettings.chunkSize, {
                  preserveBookmarks: splitSettings.preserveBookmarks
                });
                break;
                
              case SPLIT_MODES.SIZE_BASED:
                result = await splitByFileSize(file, splitSettings.maxFileSize, {
                  preserveBookmarks: splitSettings.preserveBookmarks
                });
                break;
              ...
              case SPLIT_MODES.CONTENT_BASED:
                if (!splitSettings.contentPattern.trim()) {
                  result = { success: false, error: 'Please specify a content pattern to search for' };
                } else {
                  result = await splitByContentPattern(file, splitSettings.contentPattern, {
                    preserveBookmarks: splitSettings.preserveBookmarks
                  });
                }
                break;
              ...
              case SPLIT_MODES.CUSTOM_SPLIT_POINTS:
                if (!splitSettings.customSplitPoints.trim()) {
                  result = { success: false, error: 'Please specify custom split points (e.g., 3,7,12)' };
                } else {
                  result = await splitByCustomPoints(file, splitSettings.customSplitPoints, {
                    preserveBookmarks: splitSettings.preserveBookmarks
                  });
                }
                break;
      ...
      case 'split':
        return (
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-[#E1E6EB] font-medium mb-3">
                Split Method
              </label>
              <select
              ...
              >
                <option value={SPLIT_MODES.ALL_PAGES}>Split into Individual Pages</option>
                <option value={SPLIT_MODES.PAGE_RANGES}>Split by Page Ranges</option>
                <option value={SPLIT_MODES.SPECIFIC_PAGES}>Split Specific Pages</option>
                <option value={SPLIT_MODES.CHUNKS}>Split by Page Chunks</option>
                <option value={SPLIT_MODES.SIZE_BASED}>Split by File Size</option>
                <option value={SPLIT_MODES.BOOKMARKS}>Split by Bookmarks</option>
                <option value={SPLIT_MODES.CONTENT_BASED}>Split by Content Pattern</option>
                <option value={SPLIT_MODES.CUSTOM_SPLIT_POINTS}>Custom Split Points</option>
              </select>
            </div>
            ...
            {toolSettings.split.mode === SPLIT_MODES.CHUNKS && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setToolSettings(prev => ({
                    ...prev,
                    split: { ...prev.split, chunkSize: parseInt(e.target.value) || 1 }
                  }))}
                  className="w-full px-4 py-3 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] focus:outline-none focus:border-[#00A99D]"
                />
                <p className="text-[#A0AEC0] text-sm mt-1">
            ...
            {toolSettings.split.mode === SPLIT_MODES.SIZE_BASED && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setToolSettings(prev => ({
                    ...prev,
                    split: { ...prev.split, maxFileSize: parseFloat(e.target.value) || 1 }
                  }))}
                  className="w-full px-4 py-3 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] focus:outline-none focus:border-[#00A99D]"
                />
                <p className="text-[#A0AEC0] text-sm mt-1">
            ...
            {toolSettings.split.mode === SPLIT_MODES.CUSTOM_SPLIT_POINTS && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setToolSettings(prev => ({
                    ...prev,
                    split: { ...prev.split, customSplitPoints: e.target.value }
                  }))}
                  className="w-full px-4 py-3 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] focus:outline-none focus:border-[#00A99D]"
                />
                <p className="text-[#A0AEC0] text-sm mt-1">
            ...
            {/* Output Options */}
            <div className="border-t border-[#A0AEC0]/20 pt-4">
              <h4 className="text-[#E1E6EB] font-medium mb-3">Output Options</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] focus:outline-none focus:border-[#00A99D]"
                  >
                ...
                {toolSettings.split.outputNaming === 'custom' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setToolSettings(prev => ({
                        ...prev,
                        split: { ...prev.split, customPrefix: e.target.value }
                      }))}
                      className="w-full px-3 py-2 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] focus:outline-none focus:border-[#00A99D]"
                    />
                  </div>
              ...
              <div className="mt-3">
                <label className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => setToolSettings(prev => ({
                      ...prev,
                      split: { ...prev.split, preserveBookmarks: e.target.checked }
                    }))}
                    className="w-4 h-4 text-[#00A99D] bg-[#1B212C] border-[#A0AEC0]/20 rounded focus:ring-[#00A99D] focus:ring-2"
                  />
                  <span className="text-[#E1E6EB]">Preserve bookmarks in split files</span>
                </label>
              </div>
            </div>
          </div>
                ...
                <div className="space-y-4">
                  {results.map((result, index) => (
                    ...
                    >
                          ...
                          <div>
                            ...
                            {result.success ? (
                              <div className="text-[#A0AEC0] text-sm">
                                ...
                                {result.splitFiles && (
                                  <span>Split into {result.splitFiles.length} files • </span>
                                )}
                                {result.pages && (
                                  <span>Split into {result.pages.length} pages • </span>
                                )}
                                {result.splitMode && (
                                  <span>Mode: {result.splitMode.replace('_', ' ')} • </span>
                                )}
                      ...
                      {result.success && result.splitFiles && result.splitFiles.length > 0 && (
                        <div className="mt-3 border-t border-[#A0AEC0]/20 pt-3">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="text-[#E1E6EB] text-sm font-medium">
                              Split Files ({result.splitFiles.length})
                            </h4>
                            <button
                              onClick={() => downloadAllSplitFiles(result.splitFiles)}
                              className="px-2 py-1 bg-[#00A99D]/20 hover:bg-[#00A99D]/30 text-[#00A99D] rounded text-xs transition-colors"
                            >
                              Download All
                            </button>
                          </div>
                          <div className="grid gap-2 max-h-40 overflow-y-auto">
                            {result.splitFiles.map((file, fileIndex) => (
                              <div
                                key={fileIndex}
                                className="flex items-center justify-between p-2 bg-[#1B212C] rounded-lg hover:bg-[#1B212C]/80 transition-colors"
                              >
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  <FileText className="w-4 h-4 text-[#00A99D] flex-shrink-0" />
                                  <div className="min-w-0 flex-1">
                                    <p className="text-[#E1E6EB] text-sm truncate" title={file.fileName}>
                                      {file.fileName}
                                    </p>
                                    <div className="flex items-center gap-2 text-[#A0AEC0] text-xs">
                                      {file.range && <span>Pages: {file.range}</span>}
                                      <span>•</span>
                                ...
                                <button
                                  ...
                                  className="px-2 py-1 bg-[#00A99D]/20 hover:bg-[#00A99D]/30 text-[#00A99D] rounded text-xs transition-colors flex items-center gap-1 flex-shrink-0 ml-2"
