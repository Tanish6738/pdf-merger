...
const PDFMerger = ({ onNavigate }) => {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    try {
      const fileProcessingResults = await Promise.all(acceptedFiles.map(async (file) => {
        ...
        if (validation.isValid) {
          try {
            ...
            if (error.message && error.message.includes('ChunkLoadError')) {
              console.error('ChunkLoadError detected, falling back to simple estimation');
            }
      ...
      setSelectedPages(prev => {
        ...
        fileProcessingResults.forEach(({ fileId, filePages }) => {
          newMap.set(fileId, filePages.map(page => page.pageNumber));
        });
        return newMap;
      });
      ...
      if (newFiles.some(file => file.isValid)) {
        ...
        try {
          const updatedFiles = await Promise.all(newFiles.map(async (fileItem) => {
            if (!fileItem.isValid) return fileItem;
            
            try {
              const thumbnails = await generateAllThumbnails(fileItem.file, 0.25);
              const updatedPages = fileItem.extractedPages.map((page, index) => ({
                ...page,
              ...
              console.warn(`Failed to generate thumbnails for ${fileItem.name}:`, thumbnailError);
              return fileItem;
            }
          }));
          ...
          const updatedAllPages = updatedFiles.flatMap(file => file.extractedPages);
          setAllPages(prev => {
            const updated = [...prev];
  ...
  const mergePDFs = async () => {
    ...
    const hasSelectedPages = Array.from(selectedPages.values()).some(pages => pages.length > 0);
    ...
    try {
      ...
      // Add page selections
      const pageSelections = validFiles.map(file => 
        selectedPages.get(file.id) || []
      );
      formData.append('pageSelections', JSON.stringify(pageSelections));

      // Add custom filename
      const fileName = customFileName.trim() || DEFAULT_SETTINGS.defaultFileName;
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">        {/* Header */}        <motion.div
        ...
        >
          ...
          >
            ...
            />              <div className="mb-4">
              {isProcessingFiles ? (
                <>
                  <Loader className="w-12 h-12 text-[#00A99D] mx-auto mb-3 animate-spin" />
            ...
            <div className="flex flex-wrap justify-center gap-4 text-sm text-[#A0AEC0]">
          ...
          </div>        </motion.div>        {/* View Mode Toggle */}
        {files.length > 0 && (
          <motion.div
          ...
          >
            <div className="bg-[#151B24] rounded-xl border border-[#A0AEC0]/20 p-1 flex">
              <button
            ...
            {isGeneratingThumbnails && (
              <div className="ml-4 flex items-center text-[#A0AEC0] text-sm">
          ...
          >
                ...
                <div className="space-y-3">
                  {files.map((file, index) => (
                    <SortableFileItem
                      ...
                      file={file}
                      index={index}
                      total={files.length}                      onRemove={removeFile}
            ...
            <div className="mt-6 pt-6 border-t border-[#A0AEC0]/20">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-[#00A99D]">{validFilesCount}</div>
                  <div className="text-[#A0AEC0] text-sm">Valid Files</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-[#00A99D]">{formatFileSize(totalSize)}</div>
                  <div className="text-[#A0AEC0] text-sm">Total Size</div>
                </div>                <div>
                  <div className="text-2xl font-bold text-[#00A99D]">
                    {files.reduce((sum, file) => sum + (file.pages || 0), 0)}
                  </div>
                  <div className="text-[#A0AEC0] text-sm">Total Pages</div>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        {files.length > 0 && viewMode === 'pages' && (
          ...
          >            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    ...
                    files.forEach(file => {
                      if (file.isValid) {
                        newSelectedPages.set(file.id, file.extractedPages.map(p => p.pageNumber));
                      }
                    });
                    setSelectedPages(newSelectedPages);
                  }}
                  className="px-3 py-1 text-sm bg-[#00A99D] text-white rounded hover:bg-[#00A99D]/90 transition-colors"
                >
                  Select All
                </button>
                <button
                  ...
                  className="px-3 py-1 text-sm bg-[#A0AEC0]/20 text-[#A0AEC0] rounded hover:bg-[#A0AEC0]/30 transition-colors"
                >
                  Deselect All
                </button>
                <button
            ...
            {/* Instructions */}
            <div className="mb-4 p-3 bg-[#00A99D]/10 border border-[#00A99D]/20 rounded-lg">
              <p className="text-[#E1E6EB] text-sm">
                💡 <strong>Tip:</strong> Click on pages to select/deselect them. Drag pages to reorder them in your final merged PDF.
              </p>
            </div>
            ...
            {allPages.length > 0 ? (
              ...
              >                <SortableContext items={allPages} strategy={rectSortingStrategy}>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 auto-rows-fr">
                    {allPages.map((page, index) => {
                      const isSelected = selectedPages.get(page.fileId)?.includes(page.pageNumber) || false;
                        ...
                        <PageThumbnail
                          ...
                          onToggleSelection={(pageId) => {
                            ...
                            if (page) {
                              setSelectedPages(prev => {
                                const newMap = new Map(prev);
                                const fileSelectedPages = newMap.get(page.fileId) || [];
                                const isCurrentlySelected = fileSelectedPages.includes(page.pageNumber);
                                
                                if (isCurrentlySelected) {
                                  // Remove from selection
                                  newMap.set(page.fileId, fileSelectedPages.filter(p => p !== page.pageNumber));
                                } else {
                                  // Add to selection
                                  newMap.set(page.fileId, [...fileSelectedPages, page.pageNumber]);
                                }
                                return newMap;
                              });
                            }
              ...
              <div className="text-center py-12">
                <FileText className="w-16 h-16 text-[#A0AEC0] mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-[#E1E6EB] mb-2">No Pages Available</h3>
                <p className="text-[#A0AEC0]">
            ...
            {allPages.length > 0 && (
              <div className="mt-6 pt-6 border-t border-[#A0AEC0]/20">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                  <div>
                    <div className="text-2xl font-bold text-[#00A99D]">{allPages.length}</div>
                    <div className="text-[#A0AEC0] text-sm">Total Pages</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-[#00A99D]">{getAllSelectedPages().length}</div>
                    <div className="text-[#A0AEC0] text-sm">Selected Pages</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-[#00A99D]">{validFilesCount}</div>
                    <div className="text-[#A0AEC0] text-sm">Source Files</div>
                  </div>
                </div>
              </div>
            )}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-[#E1E6EB]">Merge Settings</h3>
            <button
              onClick={() => setShowSettings(!showSettings)}
              className="text-[#A0AEC0] hover:text-[#00A99D] transition-colors"
            >
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] placeholder-[#A0AEC0] focus:outline-none focus:border-[#00A99D] transition-colors"
              />
            </div>
            ...
            <AnimatePresence>
              {showSettings && (
                <motion.div
                  ...
                  className="space-y-4 pt-4 border-t border-[#A0AEC0]/20"
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setSettings(prev => ({ ...prev, maxFiles: parseInt(e.target.value) }))}
                        className="w-full px-4 py-3 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] focus:outline-none focus:border-[#00A99D] transition-colors"
                      />
                    </div>
                    <div>
                      <label className="block text-[#E1E6EB] font-medium mb-2">
                      ...
                      <input
                        ...
                        onChange={(e) => setSettings(prev => ({ ...prev, maxFileSize: parseInt(e.target.value) * 1024 * 1024 }))}
                        className="w-full px-4 py-3 bg-[#1B212C] border border-[#A0AEC0]/20 rounded-lg text-[#E1E6EB] focus:outline-none focus:border-[#00A99D] transition-colors"
                      />
                    </div>
                  </div>
          ...
          {mergeStatus === MERGE_STATUS.PROCESSING && (
            ...
            >
              <div className="text-center mb-4">
                <Loader className="w-8 h-8 text-[#00A99D] mx-auto mb-2 animate-spin" />
                <h3 className="text-lg font-semibold text-[#E1E6EB]">Merging PDFs...</h3>
                <p className="text-[#A0AEC0]">Please wait while we combine your files</p>
              </div>
              <div className="w-full bg-[#1B212C] rounded-full h-3">
                <motion.div
                  className="bg-[#00A99D] h-3 rounded-full"
                  initial={{ width: 0 }}
                  animate={{ width: `${progress}%` }}
                  transition={{ duration: 0.3 }}
                />
              </div>
              <div className="text-center mt-2 text-[#A0AEC0]">{progress}%</div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {mergeStatus === MERGE_STATUS.COMPLETED && (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-green-400">PDFs merged successfully! Download started automatically.</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>        {/* Merge Button */}
        <motion.div
          ...
          <button
            ...
            className={`px-8 py-4 rounded-lg font-semibold text-lg transition-all duration-300 ${
              ((viewMode === 'files' && validFilesCount >= 2) || 
               (viewMode === 'pages' && getAllSelectedPages().length > 0)) && 
              mergeStatus !== MERGE_STATUS.PROCESSING
                ? 'bg-[#00A99D] hover:bg-[#00A99D]/90 text-white shadow-lg hover:shadow-xl'
